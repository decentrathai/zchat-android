...
fun NavGraphBuilder.walletNavGraph(
    ...
    navigation<MainAppGraph>(startDestination = ChatList) {
        // Chat screens - main screens for messenger
        composable<ChatList> {
            AndroidChatList()
        ...
        composable<ChatDetail> { backStackEntry ->
            val chatDetail = backStackEntry.toRoute<ChatDetail>()
            AndroidChatDetail(peerAddress = chatDetail.peerAddress)
        }
        composable<ZchatReceive> {
            AndroidZchatReceive()
        }
