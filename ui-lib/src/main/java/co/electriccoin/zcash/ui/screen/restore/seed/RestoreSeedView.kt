...
private fun Content(
    ...
    Column(
        ...
        // QR Scan buttons
        if (state.onScanCameraClick != null || state.onScanGalleryClick != null) {
            QrScanButtons(
                onScanCameraClick = state.onScanCameraClick,
                onScanGalleryClick = state.onScanGalleryClick
            )
            Spacer(Modifier.height(16.dp))
        }
...
@Composable
private fun QrScanButtons(
    onScanCameraClick: (() -> Unit)?,
    onScanGalleryClick: (() -> Unit)?
) {
    Column(
        modifier = Modifier.fillMaxWidth()
    ) {
        Text(
            text = stringResource(R.string.restore_qr_scan_title),
            style = ZashiTypography.textSm,
        ...
        Row(
            ...
            if (onScanCameraClick != null) {
                ZashiButton(
                    text = stringResource(R.string.restore_qr_scan_camera),
                    onClick = onScanCameraClick,
                    modifier = Modifier.weight(1f),
                    colors = ZashiButtonDefaults.secondaryColors()
                )
            }
            if (onScanGalleryClick != null) {
                ZashiButton(
                    text = stringResource(R.string.restore_qr_scan_gallery),
                    onClick = onScanGalleryClick,
                    modifier = Modifier.weight(1f),
                    colors = ZashiButtonDefaults.secondaryColors()
                )
            }
