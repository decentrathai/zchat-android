 ...
 * @param themeMode The theme mode to use. When null, uses forceDarkMode or system theme.
 */
...
fun ZcashTheme(
    ...
    // Determine effective theme based on themeMode or fallback to forceDarkMode/system
    val effectiveTheme = when {
        themeMode != null -> when (themeMode) {
            ThemeMode.SYSTEM -> if (isSystemDark) ThemeMode.DARK else ThemeMode.LIGHT
            else -> themeMode
        }
        forceDarkMode -> ThemeMode.DARK
        isSystemDark -> ThemeMode.DARK
